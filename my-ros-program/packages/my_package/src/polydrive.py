...
class VideoFilter(DTROS):
    ...
    def detect_duckie(self,img):
        ...
        opening = cv2.morphologyEx(yellow_image, cv2.MORPH_OPEN, kernel) 
        ...
        #remove parts of the image with straight lines
        ...
        #remove parts of the image that look like rectangles (yellow stripes)
        ...
        #draw a circle around curved objects (hopefully duckies)
    ...
    def image_cb(self, msg):
        ...
        #checks for duckies; if 2 of the last 4 frames had a duck, then stop
        ...
        #process the image for detecting line segments
        ...
        #calculates the average horizontal positioning of the lines (should be around 320 if the bot is in the center of the lane)
        line_avg = 0
        counter = 0
        for i in range(0, len(lines)):
            for x1,y1,x2,y2 in lines[i]:
                ...
                counter += 1
        line_avg /= counter
        rospy.loginfo(line_avg)
        counter=0
        self.motor_command(line_avg)
        ...
        out_msg = self.bridge.cv2_to_compressed_imgmsg(white_yellow_part)
        out_msg.header = msg.header
        ...
        #Looks for red line and stops when it approaches
        if len(contours)>0:
    ...
    def motor_command(self,line):
        wheels_cmd_msg = WheelsCmdStamped()
        wheels_cmd_msg.header.stamp = rospy.Time.now()
        wheels_cmd_msg.vel_right = 6.10352*10**(-7)*line**2
        wheels_cmd_msg.vel_left = 6.10352*10**(-7)*(line-640)**2
